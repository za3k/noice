...
nochange:
		...
		case SEL_RUNARG:
            ...
            strcat(complete_path, dents[cur].name);
			spawn(run, complete_path, path);
			initcurses();
			break;
		}
		/* Screensaver */
...
main(int argc, char *argv[])
{
	...
	if (argv[1] != NULL) {
        ...
        if (restrictdir) strcpy(chroot_wd,cwd);
